...
public class Board : NetworkBehaviour
{
    ...
    public bool ValidateMove(PlayerColour activePlayer, ChessPiece selectedChessPiece, Vector3Int tilePosition, out bool takenPiece)
    {
    ...
    public ChessPiece GetPieceFromId(uint id)
    {
    ...
    internal void HandlePawnProtionClientRpc(NetworkBehaviourReference target, ChessPieceType chessPieceType)
    {
        ...
        if (target.TryGet(out ChessPiece chessPiece))
        {
            chessPiece.ChangePieceTo(chessPieceType);
            DetectCheckServerRpc();
        }
    }
